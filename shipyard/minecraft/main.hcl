container "minecraft" {
  ...
  env {
    key   = "PROJECTOR_browserless"
    value = "${var.browserless_address}/image"
  }

  env {
    key   = "PROJECTOR_nomad"
    value = "${var.browserless_address}/image?url=http%3A//server.local.nomad-cluster.shipyard.run%3A4646"
  }

  env {
    key   = "PROJECTOR_consul"
    value = "${var.browserless_address}/image?url=http%3A//consul.container.shipyard.run%3A8500"
  }

  env {
    key   = "PROJECTOR_grafana"
    value = "${var.browserless_address}/image?url=http%3A//1.client.local.nomad-cluster.shipyard.run%3A18081"
  }
}
